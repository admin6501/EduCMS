...
ensure_dirs(){
  ...
  cd "${APP_DIR}"
  mkdir -p app/templates/{courses,accounts,orders,tickets,settings,admin,partials,wallet} app/static app/media nginx certbot/www certbot/conf
...
class Order(models.Model):
    ...
    wallet_used = models.PositiveIntegerField(default=0, verbose_name=_("مبلغ استفاده شده از کیف پول"))
    paid_with_wallet = models.BooleanField(default=False, verbose_name=_("پرداخت با کیف پول"))
...
class WalletSetting(models.Model):
    ...
    min_topup = models.PositiveIntegerField(default=10000, verbose_name=_("حداقل مبلغ شارژ (تومان)"))
    max_topup = models.PositiveIntegerField(default=20000000, verbose_name=_("حداکثر مبلغ شارژ (تومان)"))
    updated_at = models.DateTimeField(auto_now=True, verbose_name=_("بروزرسانی"))
    ...
    def __str__(self): return "Wallet Settings"
...
class Wallet(models.Model):
    ...
    balance = models.BigIntegerField(default=0, verbose_name=_("موجودی (تومان)"))
    is_active = models.BooleanField(default=True, verbose_name=_("فعال"))
    updated_at = models.DateTimeField(auto_now=True, verbose_name=_("بروزرسانی"))
    ...
    def __str__(self): return f"Wallet: {self.user_id}"

class WalletTxType(models.TextChoices):
    ADMIN_ADJUST = "admin_adjust", _("تغییر توسط ادمین")
    MASS_CREDIT = "mass_credit", _("واریز همگانی")
    TOPUP_REQUEST = "topup_request", _("درخواست شارژ")
    TOPUP_APPROVED = "topup_approved", _("تایید شارژ")
    TOPUP_REJECTED = "topup_rejected", _("رد شارژ")
    PURCHASE = "purchase", _("خرید دوره")

class WalletTxStatus(models.TextChoices):
...
class WalletTransaction(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    user = models.ForeignKey(settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="wallet_transactions", verbose_name=_("کاربر"))
    type = models.CharField(max_length=30, choices=WalletTxType.choices, verbose_name=_("نوع"))
    status = models.CharField(max_length=20, choices=WalletTxStatus.choices, default=WalletTxStatus.DONE, verbose_name=_("وضعیت"))
    amount = models.BigIntegerField(verbose_name=_("مبلغ (تومان)"))
    balance_after = models.BigIntegerField(default=0, verbose_name=_("موجودی بعد از تراکنش"))
    note = models.CharField(max_length=250, blank=True, verbose_name=_("یادداشت"))
    ...
    def __str__(self): return f"{self.user_id} {self.type} {self.amount}"

def wallet_get_or_create(user):
    w, _ = Wallet.objects.get_or_create(user=user)
    return w

@transaction.atomic
def wallet_apply(user, delta, tx_type, status=WalletTxStatus.DONE, note=""):
    w = wallet_get_or_create(user)
    w.balance = int(w.balance) + int(delta)
    w.save(update_fields=["balance","updated_at"])
    WalletTransaction.objects.create(
        user=user, type=tx_type, status=status, amount=int(delta),
        balance_after=int(w.balance), note=note
    )
    return w
PY
...
from .models import (
    BankTransferSetting, Order, OrderStatus,
    WalletSetting, wallet_get_or_create, wallet_apply, WalletTxType, WalletTxStatus
)
...
def checkout(request, slug):
    ...
    w_setting = WalletSetting.objects.first()
    wallet_enabled = bool(getattr(w_setting, "enabled", True))
    wallet = wallet_get_or_create(request.user) if wallet_enabled else None
    wallet_active = bool(wallet and wallet.is_active)
    wallet_balance = int(wallet.balance) if wallet else 0

    can_wallet_pay = wallet_enabled and wallet_active and wallet_balance >= final_amount and final_amount > 0
...
def wallet_pay(request, order_id):
    ...
    with transaction.atomic():
        wallet_apply(request.user, -amount, WalletTxType.PURCHASE, note=f"خرید دوره: {order.course.title}")
        order.status = OrderStatus.PAID
        order.paid_with_wallet = True
        order.wallet_used = amount
        order.verified_at = timezone.now()
        order.save(update_fields=["status","paid_with_wallet","wallet_used","verified_at"])
        Enrollment.objects.get_or_create(user=request.user, course=order.course, defaults={"is_active": True, "source": "wallet"})

    messages.success(request, "پرداخت با کیف پول انجام شد و دوره فعال گردید.")
    return redirect("course_detail", slug=order.course.slug)
...
def wallet_dashboard(request):
    w_setting = WalletSetting.objects.first()
    enabled = bool(getattr(w_setting, "enabled", True))
    wallet = wallet_get_or_create(request.user) if enabled else None
    txs = request.user.wallet_transactions.all()[:100] if enabled else []
    return render(request, "wallet/dashboard.html", {"enabled": enabled, "wallet": wallet, "txs": txs})
...
def wallet_topup_request(request):
    ...
    if request.method == "POST":
        ...
        min_a = int(getattr(w_setting, "min_topup", 0) or 0)
        max_a = int(getattr(w_setting, "max_topup", 0) or 0)

        if amount <= 0:
            messages.error(request, "مبلغ نامعتبر است.")
            return redirect("wallet_topup_request")
        if min_a and amount < min_a:
            messages.error(request, f"حداقل مبلغ شارژ {min_a} تومان است.")
            return redirect("wallet_topup_request")
        if max_a and amount > max_a:
            messages.error(request, f"حداکثر مبلغ شارژ {max_a} تومان است.")
            return redirect("wallet_topup_request")

        from .models import WalletTransaction
        WalletTransaction.objects.create(
            user=request.user,
            type=WalletTxType.TOPUP_REQUEST,
            status=WalletTxStatus.PENDING,
            amount=amount,
            balance_after=int(wallet.balance),
            note="درخواست شارژ"
        )
        messages.success(request, "درخواست شارژ ثبت شد. پس از تایید ادمین، موجودی شارژ می‌شود.")
        return redirect("wallet_dashboard")

    return render(request, "wallet/topup.html", {"setting": w_setting, "wallet": wallet})
PY
...
from django.utils.translation import gettext_lazy as _
from .models import (
    BankTransferSetting, Order, OrderStatus, Coupon,
    WalletSetting, Wallet, WalletTransaction, WalletTxType, WalletTxStatus,
    wallet_get_or_create, wallet_apply
)
...
class OrderAdmin(admin.ModelAdmin):
    list_display = ("id","user","course","amount","discount_amount","final_amount","wallet_used","paid_with_wallet","status","created_at")
    list_filter = ("status","created_at","paid_with_wallet")
    search_fields = ("user__username","user__email","course__title","tracking_code","coupon__code")
    actions = [mark_paid_and_enroll]

@admin.register(Coupon)
class CouponAdmin(admin.ModelAdmin):
    list_display = ("code","type","value","is_active","start_at","end_at","max_uses","max_uses_per_user","min_amount")
...
class WalletSettingAdmin(admin.ModelAdmin):
    list_display = ("enabled","min_topup","max_topup","updated_at")
...
class WalletAdmin(admin.ModelAdmin):
    ...
    actions = ["credit_selected", "debit_selected", "mass_credit"]

    @admin.action(description="افزایش موجودی کاربران انتخاب‌شده (+100,000)")
    def credit_selected(self, request, queryset):
        for w in queryset.select_related("user"):
            wallet_apply(w.user, 100_000, WalletTxType.ADMIN_ADJUST, note="افزایش توسط ادمین")
        self.message_user(request, "افزایش موجودی انجام شد.", level=messages.SUCCESS)

    @admin.action(description="کاهش موجودی کاربران انتخاب‌شده (-100,000)")
    def debit_selected(self, request, queryset):
        for w in queryset.select_related("user"):
            wallet_apply(w.user, -100_000, WalletTxType.ADMIN_ADJUST, note="کاهش توسط ادمین")
        self.message_user(request, "کاهش موجودی انجام شد.", level=messages.SUCCESS)

    @admin.action(description="واریز همگانی به همه کاربران (+50,000)")
    def mass_credit(self, request, queryset):
        from django.contrib.auth import get_user_model
        User = get_user_model()
        count = 0
        for u in User.objects.all().iterator():
            wallet_apply(u, 50_000, WalletTxType.MASS_CREDIT, note="واریز همگانی")
            count += 1
        self.message_user(request, f"واریز همگانی برای {count} کاربر انجام شد.", level=messages.SUCCESS)

@admin.register(WalletTransaction)
class WalletTransactionAdmin(admin.ModelAdmin):
    list_display = ("created_at","user","type","status","amount","balance_after","note")
...
from django.urls import path
from .views import checkout, upload_receipt, my_orders, wallet_dashboard, wallet_topup_request, wallet_pay
urlpatterns = [
    ...
    path("wallet/topup/", wallet_topup_request, name="wallet_topup_request"),
    path("wallet/pay/<uuid:order_id>/", wallet_pay, name="wallet_pay"),
]
PY
  ...
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between gap-4">
    ...
    <div class="flex items-center gap-2 text-sm">
      ...
      {% if user.is_authenticated %}
        <a class="px-3 py-1 rounded-xl hover:underline" href="/orders/wallet/">کیف پول</a>
...
<div class="max-w-md mx-auto bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 p-6">
  ...
  <form method="post">{% csrf_token %}{{ form.as_p }}
    <button class="mt-3 w-full px-4 py-2 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900">ورود</button>
  </form>
</div>
{% endblock %}
HTML
...
<div class="max-w-md mx-auto bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 p-6">
  ...
  <form method="post">{% csrf_token %}{{ form.as_p }}
    <button class="mt-3 w-full px-4 py-2 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900">ساخت حساب</button>
  </form>
</div>
{% endblock %}
HTML
...
<div class="max-w-2xl mx-auto bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 p-6">
  ...
  <form method="post" class="mb-4">{% csrf_token %}<input type="hidden" name="action" value="apply_coupon"/>
  ...
  <div class="p-4 rounded-xl border dark:border-slate-700 bg-slate-50 dark:bg-slate-950 mb-4 space-y-1">
    ...
    {% if wallet_enabled %}
      <div class="p-4 rounded-xl border dark:border-slate-700 bg-slate-50 dark:bg-slate-950 mb-4">
        <div class="text-sm">موجودی کیف پول: <b>{{ wallet_balance }}</b> تومان</div>
        {% if not wallet_active %}
          <div class="text-xs text-slate-500 dark:text-slate-300 mt-1">کیف پول شما غیرفعال است.</div>
        {% endif %}
        {% if can_wallet_pay %}
          <a class="inline-block mt-3 px-4 py-2 rounded-xl border dark:border-slate-700 bg-emerald-600 text-white"
             href="/orders/wallet/pay/{{ order.id }}/">پرداخت کامل با کیف پول</a>
          <div class="text-xs text-slate-500 dark:text-slate-300 mt-1">پرداخت با کیف پول نیازی به آپلود رسید ندارد.</div>
        {% else %}
          <div class="text-xs text-slate-500 dark:text-slate-300 mt-2">برای پرداخت با کیف پول، باید موجودی کافی داشته باشید.</div>
        {% endif %}
        <a class="inline-block mt-3 px-4 py-2 rounded-xl border dark:border-slate-700" href="/orders/wallet/topup/">درخواست شارژ کیف پول</a>
      </div>
    {% endif %}
...
HTML
  ...
  cat > app/templates/wallet/dashboard.html <<'HTML'
{% extends "base.html" %}
{% block title %}کیف پول{% endblock %}
{% block content %}
<div class="max-w-4xl mx-auto bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 p-6">
  <h1 class="text-xl font-bold mb-4">کیف پول</h1>

  {% if not enabled %}
    <div class="p-4 rounded-xl border dark:border-slate-700">کیف پول در حال حاضر غیرفعال است.</div>
  {% else %}
    <div class="p-4 rounded-xl border dark:border-slate-700 bg-slate-50 dark:bg-slate-950 mb-6">
      <div class="text-sm">موجودی: <b>{{ wallet.balance }}</b> تومان</div>
      <div class="text-sm mt-1">وضعیت: <b>{% if wallet.is_active %}فعال{% else %}غیرفعال{% endif %}</b></div>
      <a class="inline-block mt-3 px-4 py-2 rounded-xl border dark:border-slate-700" href="/orders/wallet/topup/">درخواست شارژ</a>
    </div>

    <h2 class="font-semibold mb-3">آخرین تراکنش‌ها</h2>
    <div class="space-y-3">
      {% for t in txs %}
        <div class="p-4 rounded-xl border dark:border-slate-700">
          <div class="flex flex-wrap items-center justify-between gap-2">
            <div class="text-sm"><b>{{ t.get_type_display }}</b> — {{ t.get_status_display }}</div>
            <div class="text-sm" dir="ltr">{{ t.created_at }}</div>
          </div>
          <div class="text-sm mt-1">مبلغ: <b>{{ t.amount }}</b> تومان</div>
          <div class="text-xs text-slate-500 dark:text-slate-300 mt-1">موجودی بعد: {{ t.balance_after }} تومان</div>
          {% if t.note %}<div class="text-xs text-slate-500 dark:text-slate-300 mt-1">{{ t.note }}</div>{% endif %}
        </div>
      {% empty %}
        <div class="text-slate-600 dark:text-slate-300">تراکنشی ثبت نشده است.</div>
      {% endfor %}
    </div>
  {% endif %}
</div>
...
{% extends "base.html" %}
{% block title %}درخواست شارژ کیف پول{% endblock %}
{% block content %}
<div class="max-w-xl mx-auto bg-white dark:bg-slate-900 rounded-2xl border dark:border-slate-800 p-6">
  <h1 class="text-xl font-bold mb-4">درخواست شارژ کیف پول</h1>

  <div class="p-4 rounded-xl border dark:border-slate-700 bg-slate-50 dark:bg-slate-950 mb-4">
    <div class="text-sm">موجودی فعلی: <b>{{ wallet.balance }}</b> تومان</div>
    {% if setting %}
      <div class="text-xs text-slate-500 dark:text-slate-300 mt-2">
        حداقل: {{ setting.min_topup }} تومان — حداکثر: {{ setting.max_topup }} تومان
      </div>
    {% endif %}
  </div>

  <form method="post">{% csrf_token %}
    <label class="block text-sm mb-2">مبلغ (تومان)</label>
    <input name="amount" inputmode="numeric" class="w-full px-3 py-2 rounded-xl border dark:border-slate-700 bg-white dark:bg-slate-950" placeholder="مثلاً 100000" />
    <button class="mt-4 w-full px-4 py-2 rounded-xl bg-slate-900 text-white dark:bg-white dark:text-slate-900">ثبت درخواست</button>
  </form>

  <div class="mt-4 text-xs text-slate-500 dark:text-slate-300">
    پس از ثبت درخواست، ادمین از طریق پنل مدیریت آن را بررسی و تایید می‌کند.
  </div>
</div>
{% endblock %}
HTML
...
from payments.models import BankTransferSetting, WalletSetting
...
WalletSetting.objects.get_or_create(id=1)
...
do_install(){
  ...
  echo "Wallet:  https://${DOMAIN}/orders/wallet/"
