...
#   ╚══════╝╚═════╝  ╚═════╝  ╚═════╝╚═╝     ╚═╝╚══════╝
#
#   EduCMS - سیستم مدیریت آموزش حرفه‌ای
...
#   ├── 💳 پرداخت آنلاین (زرین‌پال، زیبال، آقای پرداخت)
#   ├── 👛 کیف پول اختصاصی کاربران
#   ├── 🎫 سیستم تیکت پشتیبانی
#   ├── 🛡️ امنیت IP پیشرفته با Rate Limiting
...
#   ├── Frontend: Tailwind CSS + RTL Support
#   ├── Database: MySQL 8.0
#   ├── Server:   Nginx + Gunicorn
#   └── Deploy:   Docker Compose
#
#   الگوهای طراحی استفاده شده:
#   ├── Strategy Pattern    → سیستم درگاه پرداخت
#   ├── Service Layer       → IPSecurityService
...
readonly LOG_FILE="/var/log/educms-install.log"
...
try:
    ...
    # ===== ONLINE PAYMENT =====
    # Check if table exists with wrong schema and drop it (user_id should be CHAR(36) for UUID)
    if table_exists(cursor, 'payments_onlinepayment'):
        ...
        if row:
            ...
            # If user_id is NOT CHAR(36), drop and recreate (UUID requires CHAR(36))
            if 'char' not in col_type:
                print("Dropping payments_onlinepayment table (wrong user_id type, should be CHAR(36) for UUID)...")
...
backup_db(){
  ...
  # Note: --set-gtid-purged is not supported in MariaDB
  # --skip-ssl to avoid self-signed certificate issues (MariaDB compatible)
  if docker compose exec -T -e MYSQL_PWD="${DB_PASS}" db mysqldump -uroot --skip-ssl --databases "${DB_NAME}" --single-transaction --quick --routines --triggers --events > "$file" 2>/dev/null; then
    if [[ -s "$file" ]]; then
      chmod 600 "$file"
...
menu_header(){
  ...
  echo ""
  echo "  ═══════════════════════════════════════════════════"
  echo "    سیستم مدیریت آموزش حرفه‌ای | نسخه ${VERSION}"
  echo "  ═══════════════════════════════════════════════════"
  echo ""
...
menu_show(){
  echo "  ┌─────────────────────────────────────────────┐"
  echo "  │           گزینه‌های مدیریت سیستم            │"
  echo "  ├─────────────────────────────────────────────┤"
  echo "  │  1) 🚀 Install     نصب کامل                 │"
  echo "  │  2) 🔄 Patch       بروزرسانی کد             │"
  echo "  │  3) ▶️  Start       استارت سرویس‌ها          │"
  echo "  │  4) ⏹️  Stop        توقف سرویس‌ها            │"
  echo "  │  5) 🔁 Restart     ری‌استارت                │"
  echo "  │  6) 💾 Backup      بکاپ دیتابیس             │"
  echo "  │  7) 📥 Restore     ریستور بکاپ              │"
  echo "  │  8) 🌐 Domain      تغییر دامنه              │"
  echo "  │  9) 🔐 Password    تغییر رمز ادمین          │"
  echo "  │ 10) 🗑️  Uninstall   حذف کامل                │"
  echo "  │  0) ❌ Exit        خروج                     │"
  echo "  └─────────────────────────────────────────────┘"
  echo ""
}
